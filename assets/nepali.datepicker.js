...
var s=$("#"+t).position()
$("#ndp-nepali-box").css("top",s.top+$("#"+t).outerHeight()),$("#ndp-nepali-box").css("left",s.left),showCalendar(e)}function setSelectedDay(t){var e=$("#ndp-target-id").html()
$("#"+e).val(t),hideCalendarBox()}function showCalendar(t){$("#ndp-nepali-box table").find("tr:gt(0)").remove(),""==t?$("#ndp-nepali-box table").append(getDateTable("")):$("#ndp-nepali-box table").append(getDateTable(t)),"block"==$("#ndp-nepali-box").css("display")&&$("#ndp-nepali-box").hide(),$("#ndp-nepali-box").fadeIn(100)}function getDateTable(t){if(""==t){var e=getNepaliDate(),s=getMonthParameters(e),a=getDateRows(s[0],s[1],s[2],s[3],s[4])
